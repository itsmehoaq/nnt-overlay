    ...
    if (tempMapID !== data.menu.bm.id && data.menu.bm.id != 0) {
        if (tempMapID == 0) tempMapID = data.menu.bm.id;
            ...
            if (pickedMap && enableAutoPick && !selectedMaps.includes(tempMapID))
                pickMap(
                    Array.from(beatmaps).find((b) => b.beatmapID == tempMapID),
...
async function setupBeatmaps() {
    ...
    bms.map(async (beatmap, index) => {
        ...
        bm.clicker.style.backgroundImage = `url('https://assets.ppy.sh/beatmaps/${mapData.beatmapset_id}/covers/cover.jpg')`;
...
const pickMap = (bm, teamName, color) => {
    ...
    // setTimeout(() => {
        ...
        bm.pickedStatus.style.outline = bm.mods.includes("TB") ? "5px solid #ffffff" : `5px solid ${color == "Red" ? "#f51651" : "#1e88fa"}`;
...
const banMap = (bm, teamName, color) => {
    ...
    // setTimeout(() => {
        ...
        bm.pickedStatus.innerHTML = `<b class="pick${color}">BANNED</b>`;
    // }, 300);
};
